...
@WebServlet("/GetInvitati")
public class GetInvitati extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Utente> listaInvitati = new ArrayList<Utente>();
		
		try {
			listaInvitati = uDAO.trovaPersoneDaInvitare(utente.getId());
		} catch(SQLException e) {
			e.printStackTrace();
			response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			response.getWriter().println("Errore nel recuperare la lista delle persone da invitare!");
			return;
		}
		
		Gson gson = new Gson();
		String json = gson.toJson(listaInvitati);	//uso gson per serializzare
